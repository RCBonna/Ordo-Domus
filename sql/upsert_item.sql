-- ============================================================
-- upsert_inventario: Upsert determinístico de itens
-- Executar no SQL Editor do Supabase (supabase.com > SQL Editor)
--
-- OBJETIVO: Substituir a 2ª chamada à IA Gemini por uma lógica
-- de banco que decide automaticamente se deve SOMAR a quantidade
-- a um item existente ou CRIAR um novo registro.
--
-- MATCH: Dois itens são considerados iguais se compartilham
-- (unidade_id, nome, comodo, armario, caixa, validade)
-- comparados de forma case-insensitive e null-safe.
-- ============================================================
...
AS $$
DECLARE
  -- Versões normalizadas apenas para comparação
...
BEGIN
  ...
  -- Busca item existente (case-insensitive, null-safe, ignora soft-deleted)
  ...
  IF v_existing_id IS NOT NULL THEN
    ...
    v_acao := 'MERGE';
  ELSE
    -- ADD: insere novo item (armazena com formatação original recebida)
